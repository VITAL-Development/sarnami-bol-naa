...
- Every `content/sarnami/vocab/*.json` item: new optional `devanagari` field,
  the mechanical Devanagari rendering of that item's `word` (via
  `toDevanagari()`, `scripts/devanagari-transliterate.mjs`), generated by
  `scripts/generate-devanagari-table.mjs --write` and kept fresh in CI by two
  jobs: `validate-devanagari` (field matches the local transliterator) and
  `devanagari-drift-check` (the local transliterator hasn't diverged from
  rarelang-server's hand-synced copy that the realtime TTS pipeline actually
  runs). Display/review data only — it does not change what
  [#300](https://github.com/VITAL-Development/sarnami-bol-naa/issues/300)'s
  ElevenLabs batch sends or computes (still the romanized `word`,
  transliterated server-side per that issue's D2) — added so
  rarelang-pwa's `/dev/transliteration` page can show the currently-committed
  spelling for review before that batch spends ElevenLabs tokens on it.
  Consumers that don't recognize `devanagari` ignore it.
